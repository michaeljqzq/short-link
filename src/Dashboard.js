...
class Dashboard extends Component {
    ...
    this.fetchItemsFromServer()
      .then(results => {
        if (results.status === 401) {
          this.props.history.push(`${constant.reactLoginPath}`);
        } else {
          return results.json();
        }
      })
  ...
  fetchItemsFromServer = (page, type, search) => {
    ...
    return fetch(`/${constant.apiPath}?${queryString.stringify(condition)}`, {
      headers: {
  ...
  onSubmit = () => {
    ...
    fetch(`/${constant.apiPath}/${encodeURIComponent(this.state.keyw)}`, {
      method: 'POST',
      body,
      headers: headers,
  ...
  deleteItem = (keyw) => {
    fetch(`/${constant.apiPath}/${encodeURIComponent(keyw)}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
...
export default withRouter(({history}) => <Dashboard history={history} />);
